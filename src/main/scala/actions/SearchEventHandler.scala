...
class SearchEventHandler(controller: Controller, db: PostgresProfile.backend.Database, tableView: TableView[TaskItem]) extends EventHandler[ActionEvent] {
  override def handle(e: ActionEvent): Unit ={
    ...
    tableView.getItems.stream().filter(item => (item.task.toLowerCase().contains(textToSearch) ||
      item.comments.toLowerCase().contains(textToSearch) || item.deadline.toLowerCase.contains(textToSearch))).findAny().ifPresent(item
